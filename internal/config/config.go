...
type Config struct {
	...
	DBUser        string `env:"DB_USER" envDefault:"ldwelfaremall_user"`
	DBPassword    string `env:"DB_PASSWORD" envDefault:"ldwelfaremall_pwd"`
	DBName        string `env:"DB_NAME" envDefault:"ldwelfaremall_db"`
	...
	CORSOrigins   string `env:"APP_CORS_ORIGINS" envDefault:"http://localhost:28505"`
}
...
func splitAndTrim(raw string) []string {
	if strings.TrimSpace(raw) == "" {
		return []string{"http://localhost:28505"}
	}
